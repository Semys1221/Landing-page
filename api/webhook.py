...
def handle_smartlead():
    try:
        ...
        if category == "INTERESSE" and campaign_id and email_stats_id:
            # T1 — immédiat
            send_reply(campaign_id, email_stats_id, T1)
            # T2 — J+1 à 10h Paris
            send_reply(campaign_id, email_stats_id, T2, get_scheduled_time(1))
            # T3 — J+3 à 10h Paris
            send_reply(campaign_id, email_stats_id, T3, get_scheduled_time(3))

            discord_payload = {"embeds": [{"title": f"✅ Prospect chaud : {campaign_name}",
                "description": f"**De :** {lead_email}\n**Message :** {message_body}\n\n📧 T1 envoyé, T2 planifié J+1, T3 planifié J+3",
                "color": 3066993}]}
